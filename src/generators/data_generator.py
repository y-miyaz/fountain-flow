...
class DataGenerator:
    ...
    def generate_data_to_table(self, table_name):
        try:
            ...
            table_config = [table_config for table_config in self.data_config['tables'] if table_config['name'] == table_name]
                ...
                for index in range(row_count):
                    ...
                    if (index + 1) % self.csv_batch_size == 0 or index == table_config['row_count'] - 1:
                        try:
                            ...
                            is_error = True
                            break
                path = Path(csv_writer.file_path)
                abs_path = path.resolve(strict=True)
                logging.info(
                    f"Data generation completed: '{table_config['row_count']}' records were written to the file '{abs_path}'.")
            if is_error is True:
                raise
        except Exception:
            raise
    ...
    def load_csv_data(self, table_name, file_path, include_headers: bool):
        try:
            ...
            abs_path = path.resolve(strict=True)
            logging.info(
                f"Successfully load '{abs_path}' into '{table_name}'.")
        except Exception:
            raise
        finally:
            db_connector.close()
                ...
                for index in range(row_count):
                    ...
                    if (index + 1) % self.csv_batch_size == 0 or index == table_config['row_count'] - 1:
                        try:
                            ...
                            is_error = True
                            break
                path = Path(json_writer.file_path)
                abs_path = path.resolve(strict=True)
                logging.info(
                    f"Data generation completed: '{table_config['row_count']}' records were written to the file '{abs_path}'.")
                response = input(
